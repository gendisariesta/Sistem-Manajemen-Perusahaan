...
export default async function Dashboard() {
  ...
  const resUser = await fetch(`${process.env.NEXT_PUBLIC_VERCEL_URL}/auth`, {
  ...
  const [resPresensi, resTugas] = await Promise.all([
    fetch(`${process.env.NEXT_PUBLIC_VERCEL_URL}/kehadiran?id_user=${dataUser.uid}`,{cache:'no-store'}),
    fetch(`${process.env.NEXT_PUBLIC_VERCEL_URL}/tugas?assign_to_id=${dataUser.uid}`,{cache:'no-store'}),
  ]);
